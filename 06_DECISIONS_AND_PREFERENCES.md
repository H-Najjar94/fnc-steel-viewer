...
- **Decision:** Madar has only DWG/DXF/Excel (no IFC/NC1). DWG can't be rendered by
  any free in-browser library (closed format). So: (a) 3D model → export to STL via
  the installed **AutoCAD 2026 headless console** (`accoreconsole` + STLOUT, free),
  render with three STLLoader; (b) 2D drawings → **LibreDWG** (free, GPL) DWG→DXF,
  render in the drawing tab. User chose "do both".
...
- **Gotcha recorded:** DSTV weight field is kg/m for profiles but kg/m² for plates.
